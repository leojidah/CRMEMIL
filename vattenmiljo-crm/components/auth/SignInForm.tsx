...
import { Eye, EyeOff, Mail, Lock, User, Building2, Wrench, AlertCircle } from 'lucide-react'
...
export default function SignInForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        setError('Ogiltigt email eller lösenord')
      } else if (data.user) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          <div className="text-center">
            ...
            <p className="text-gray-600">
              Logga in på ditt Vattenmiljö CRM-konto
            </p>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            <p className="text-xs text-gray-500 text-center mb-3">Demo-roller:</p>
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="text-center p-2 bg-blue-50 rounded-lg">
                <User className="w-4 h-4 text-blue-600 mx-auto mb-1" />
                <div className="font-medium text-blue-900">Säljare</div>
              </div>
              <div className="text-center p-2 bg-purple-50 rounded-lg">
                <Building2 className="w-4 h-4 text-purple-600 mx-auto mb-1" />
                <div className="font-medium text-purple-900">Intern</div>
              </div>
              <div className="text-center p-2 bg-green-50 rounded-lg">
                <Wrench className="w-4 h-4 text-green-600 mx-auto mb-1" />
                <div className="font-medium text-green-900">Montör</div>
              </div>
            </div>
          </div>
        </div>
